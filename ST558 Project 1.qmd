...
Creating middle time value

```{r}

process_JWAP_data <- function() {


  #Converting JWAP to wide format using separate_wider_delim() 
  JWAP_wide <- separate_wider_delim(JWAP_tibble, 
                                    cols = c("JWAP"), 
                                    delim = " ", 
                                    names = c("start", "meridiem_1", "to", "end", "meridiem_2"), 
                                    too_few = "debug", 
                                    names_repair = "unique", 
                                    too_many = "drop") 

  #selecting the needed rows and using parse_hm to convert to usable format
  JWAP_intervals <- JWAP_wide |>
    select(id, start, meridiem_1, end, meridiem_2) |>
       mutate(start = parse_hm(start),
              end = parse_hm(end))  
              

  #Making row 1 display NA across all columns for row 1    
  JWAP_intervals[1, ] <- NA
  ...
  return(JWAP_intervals)         
}
```
 
 Saving this incase I need to reference later:
  #get the JWAP info
  ...
  JWAP_vals <- JWAP[sort(names(JWAP))]

  JWAP_vals <- JWAP[sort(names(JWAP))]
  
  

  #Converting JWAP_vals into a tibble
  JWAP_tibble <- tibble(id = 1:286, JWAP = JWAP_vals)
  
  
    if ("JWAP" %in% colnames(tibble_data)) {
    tibble_data <- process_JWAP_data(tibble_data)
  }
...
result <- query_census_pums(
  year = 2022, 
  numeric_vars = c("AGEP", "PWGTP", "GASP"),
